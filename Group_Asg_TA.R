...
##==========================  Part C ==============================##
##-----------------------------------------------------------------##
data_for_partc_all_cities <- airbnb_df %>% 
  select(reviewer_id,comments, review_scores_rating, listing_id,
         price, city_typed) %>% 
  mutate(price = as.numeric(gsub("\\$","",price)))

city_index <- "amsterdam"
data_for_partc <- data_for_partc_all_cities[data_for_partc_all_cities$city_typed %in% city_index,]

#####clean up for part c
data_for_partc$review_length_chars <- nchar(data_for_partc$comments)

hist(data_for_partc$review_length_chars,breaks = 200,main = "Review Length(All)")

data_for_partc <- data_for_partc %>% 
  filter(review_length_chars>144)

hist(data_for_partc$review_length_chars,breaks = 200,main = "Review Length(All) -Left trim to 144")

data_for_partc <- data_for_partc %>% 
  filter(review_length_chars<1000)

hist(data_for_partc$review_length_chars,breaks = 200,main = "Review Length(All) -Right trim to 1000")


to_remove_listing_ids <- data_for_partc %>% 
  group_by(listing_id) %>% 
  summarise(total = n()) %>% 
...
data_for_partc$comments <- gsub('[[:digit:]]+',' ', data_for_partc$comments)

data_for_partc$comments <- gsub('[[:punct:]]+',' ', data_for_partc$comments)

split_size <- 10000
tokens_list <- split(data_for_partc, 
                     rep(1:ceiling(nrow(data_for_partc)
                                   /split_size), 
                         each=split_size,
...
tokens_all %>% group_by(token_length) %>% 
  summarise(total =n()) %>% 
...
for_pos_list <- split(data_for_partc, 
                      rep(1:ceiling(nrow(data_for_partc)
                                    /split_size), 
                          each=split_size,
                          length.out=nrow(data_for_partc)))
...
annotated_reviews_all$reviewer_id <- as.integer(annotated_reviews_all$reviewer_id)

annotated_reviews_all %>% left_join(data_for_partc) -> data_for_partc

library(stm)

processed <- textProcessor(data_for_partc$annotated_comments,
                           metadata = data_for_partc,
...
topic_labels <- c("Place","Apartment","Room",
                  "Great","Room")
